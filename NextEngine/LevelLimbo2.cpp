...
void LevelLimbo2::levelInit() {
	...
	InteractableObject* portal_ = new InteractableObject("Portal");
	portal_->getTransform().setScale(glm::vec3(2.56f * 2.5f, 3.37f * 2.5f, 1));
	portal_->getTransform().setPosition(glm::vec3(4.75f, -0.0f, 1));
	portal_->addColliderComponent();
	portal_->getColliderComponent()->setTrigger(true);
	portal_->getColliderComponent()->setDimension(0.25f, 1.0f);
	portal_->initAnimation(19, 1);
	portal_->getAnimationComponent()->addState("idle", 0, 19);
	//portal_->getAnimationComponent()->addState("highlight", 0, 8);
	objectsList.push_back(portal_);
	portal = portal_;

	InteractableObject* dummy_ = new InteractableObject("Dummy");
	dummy_->getTransform().setScale(glm::vec3(0.64f * 2.25f, 0.8f * 2.25f, 1));
	dummy_->getTransform().setPosition(glm::vec3(-2.5f, -2.0f, 1));
	dummy_->addColliderComponent();
	dummy_->getColliderComponent()->setTrigger(true);
	dummy_->getColliderComponent()->setDimension(0.5f, 0.5f);
	dummy_->getColliderComponent()->getTransform().setPosition(glm::vec3(0.0f, 0.0f, 1));
	dummy_->setCanHit(true);
	dummy_->initAnimation(1, 1);
	objectsList.push_back(dummy_);
	dummy = dummy_;
	...
	player->getTransform().setPosition(glm::vec3(-7.5f, -0.8f, 0.0f));
...
void LevelLimbo2::levelUpdate() {
	...
	//portal->selfUpdate(playerDT);
	//dummy->selfUpdate(playerDT);
	if (dummy->getPlayerInside()) {
		dummy->setTexture("../Resource/Texture/Interactables/Dummy_Outlined.png");
	}
	else {
		dummy->setTexture("../Resource/Texture/Interactables/Dummy.png");
	}

	if (portal->getPlayerInside()) {
		if (portal2) {
			portal2->setDraw(true);
			if (GameEngine::getInstance()->getInputHandler()->getLastInput() == InputDevice::CONTROLLER) {
				portal2->setTexture("../Resource/Texture/UI/RB.png");
			}
			else {
				portal2->setTexture("../Resource/Texture/UI/E.png");
			}
		}
	}
	else {
		portal2->setDraw(false);
	}
	//handleObjectCollision(objectsList);

}

void LevelLimbo2::levelDraw() {
	GameEngine::getInstance()->render(objectsList);
	//cout << "Draw Level" << endl;
}

void LevelLimbo2::levelFree() {
...
void LevelLimbo2::levelUnload() {
	GameEngine::getInstance()->clearMesh();
	//cout << "Unload Level" << endl;
}

void LevelLimbo2::isReceivingNoInputs() {
...
void LevelLimbo2::switchControlType(ControlType ct) {
	switch (ct) {
...
void LevelLimbo2::handleKey(char key) {
	...
	case 'e':
		if (portal->getPlayerInside()) {
			if (player->getWeaponType() != WeaponType::None_) {
				SoundManager::GetInstance()->PlaySFX("Misc-PortalEnter");
				nextLevel();
			}
		}
...
void LevelLimbo2::handleControllerButton(SDL_GameControllerButton button) {
	float dt = GameEngine::getInstance()->getTime()->getDeltaTime();
	float speed = 20.0f;
	speed *= dt;
...
void LevelLimbo2::handleMouse(int type, int x, int y) {
	float realX, realY;
...
void LevelLimbo2::handleAnalogStick(int type, char key) {
...
void LevelLimbo2::addObject(DrawableObject* obj) {
	objectsList.push_back(obj);
}

void LevelLimbo2::addPlayerToDebug(Player* player) {

}

void LevelLimbo2::createSkillsIcon() {
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	//										Bow Skills Icon
	//
	...
	TexturedObject* portal1 = new TexturedObject("tutorialOverlay");
	portal1->setTexture("../Resource/Texture/UI/TutorialOverlay/TutorialOverlay-Ctrl.png");
	portal1->getTransform().setScale(glm::vec3(0.18f * 2.0f, 0.18f * 2.0f, 1.0f));
	portal1->getTransform().setPosition(glm::vec3(portal->getTransform().getPosition().x, portal->getTransform().getPosition().y + 1.0f, 1.0f));
	portal1->drawLayer = 10;
	objectsList.push_back(portal1);
	portal2 = portal1;
}

void LevelLimbo2::updateSkillsIcon() {
...
void LevelLimbo2::playerUltimateInput() {
...
void LevelLimbo2::stateMachineUpdate(float dt) {
	...
	if (portal->getStateMachine()) {
		portal->getStateMachine()->changeState(PortalIdleState::getInstance(), portal);
		portal->getStateMachine()->update(portal, dt);

		if (portal->getAnimationComponent()) {
			portal->getAnimationComponent()->updateCurrentState(dt);
		}
	}
}

bool LevelLimbo2::isPausing(float dt) {
...
void LevelLimbo2::changeMenuState(MenuState targetState) {
...
MenuState LevelLimbo2::getMenuState() const {
	return currentMenuState;
}

void LevelLimbo2::mouseUIHandling(int type, float x, float y) {
	float dt = GameEngine::getInstance()->getTime()->getDeltaTime();
...
void LevelLimbo2::keyboardUIHandling(char key) {
	float dt = GameEngine::getInstance()->getTime()->getDeltaTime();
...
glm::vec2 LevelLimbo2::convertMouseToGameSpace(int mouseX, int mouseY) {
...
void LevelLimbo2::changeSelection(int direction) {
...
void LevelLimbo2::createPauseUI() {
...
void LevelLimbo2::controllerUIHandling(SDL_GameControllerButton button) {
	float dt = GameEngine::getInstance()->getTime()->getDeltaTime();
