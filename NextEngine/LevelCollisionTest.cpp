...
void LevelCollisionTest::levelInit() {
	...
	SimpleObject* obj = new SimpleObject("player");
	...
	mapLoader.readData("map6.txt");
	mapLoader.appendDataToScene(objectsList, player);
	mapLoader.offsetMapToCenter(objectsList);
	
	SimpleObject* physObj = new SimpleObject("PhysicsObject");
	physObj->getTransform().setPosition(-3,1);
	physObj->addColliderComponent();
	physObj->addPhysicsComponent();
	physObj->getPhysicsComponent()->setEnableGravity(false);
	physObj->setDrawCollider(true);
	objectsList.emplace_back(physObj);
	...
	player->getColliderComponent()->setTrigger(false);
	player->getPhysicsComponent()->setEnableGravity(true);
}
