...
class LevelCredits : public Level
{
private:
	...
	float pictureHeight = 36.05f;
	float screenHeight = 9.0f;
	float totalTravelDistance = screenHeight + pictureHeight;
	float t = 5.0f;

	float velocityY = totalTravelDistance / t;  // -8.11f units/sec
	//float startY = -(screenHeight / 2.0f) - (pictureHeight / 2.0f);  // 20.275f
	float startY = (screenHeight / 2.0f) - (pictureHeight / 2.0f);
	float endY = -(screenHeight / 2.0f) + (pictureHeight / 2.0f);  // +22.525f

	TexturedObject* credits = nullptr;
